...
async def delete_contact_message(msg_id: str, admin: TokenData = Depends(require_permission("contact-messages"))):
...
async def integration_status(admin: TokenData = Depends(get_superadmin)):
    ...
    wa_url = os.environ.get("WA_SERVICE_URL", "https://wa.sdpublic.org")
...
async def create_gdrive_folder_admin(payload: Dict[str, Any] = Body(...), admin: TokenData = Depends(require_permission("gallery"))):
    ...
    cover_file_id = payload.get("cover_file_id", "").strip()
    selected_files = payload.get("selected_files", [])

    if not drive_url and not selected_files and not payload.get("manual_urls"):
        raise HTTPException(status_code=400, detail="Google Drive folder URL or photo links required")
    ...
    if selected_files and isinstance(selected_files, list) and len(selected_files) > 0:
        for f in selected_files:
            fid = f.get("file_id") if isinstance(f, dict) else str(f)
            if fid and fid not in seen_ids:
                seen_ids.add(fid)
                if isinstance(f, dict):
                    final_files.append({
                        "file_id": fid,
                        "title": f.get("title") or f"Photo #{len(final_files) + 1}",
                        "proxy_url": f"/api/gdrive-proxy/{fid}",
                        "download_url": f"/api/gdrive-download/{fid}"
                    })
                else:
                    final_files.append({
                        "file_id": fid,
                        "title": f"Photo #{len(final_files) + 1}",
                        "proxy_url": f"/api/gdrive-proxy/{fid}",
                        "download_url": f"/api/gdrive-download/{fid}"
                    })
    else:
        # Extract files from Google Drive folder URL
        extracted_files = await extract_gdrive_folder_files(drive_url) if drive_url else []
    ...
    if not cover_file_id and final_files:
        cover_file_id = final_files[0].get("file_id", "")
...
@admin_router.put("/gdrive-folders/{slug}")
async def update_gdrive_folder_admin(slug: str, payload: Dict[str, Any] = Body(...), admin: TokenData = Depends(require_permission("gallery"))):
    """Update album details, title, description, cover photo, and photo selections."""
    from datetime import datetime, timezone
    ...
    title = payload.get("title", "").strip() or folder.get("title")
    description = payload.get("description", "").strip()
    drive_url = payload.get("drive_folder_url", "").strip() or folder.get("drive_folder_url", "")
    cover_file_id = payload.get("cover_file_id", "").strip() or folder.get("cover_file_id", "")
    selected_files = payload.get("selected_files", None)
    ...
    if selected_files is not None and isinstance(selected_files, list):
        final_files = []
        seen_ids = set()
        for f in selected_files:
            fid = f.get("file_id") if isinstance(f, dict) else str(f)
        ...
        update_fields["files"] = final_files
        update_fields["file_count"] = len(final_files)
        if not cover_file_id and final_files:
            update_fields["cover_file_id"] = final_files[0].get("file_id", "")

    await db.gdrive_folders.update_one({"$or": [{"slug": slug}, {"id": slug}]}, {"$set": update_fields})
    updated_doc = await db.gdrive_folders.find_one({"$or": [{"slug": slug}, {"id": slug}]}, {"_id": 0})
    return updated_doc
...
async def resync_gdrive_folder_admin(slug: str, admin: TokenData = Depends(require_permission("gallery"))):
    ...
    if extracted_files:
        existing_files = folder.get("files", [])
        existing_ids = {f.get("file_id") for f in existing_files if f.get("file_id")}
        
        # Merge new extracted files
        all_files = list(existing_files)
        for ef in extracted_files:
            if ef.get("file_id") not in existing_ids:
                existing_ids.add(ef.get("file_id"))
                all_files.append(ef)

        await db.gdrive_folders.update_one(
            {"$or": [{"slug": slug}, {"id": slug}]},
            {"$set": {"files": all_files, "file_count": len(all_files), "updated_at": datetime.now(timezone.utc).isoformat()}}
        )
        return {"status": "synced", "count": len(all_files), "files": all_files}
    return {"status": "no_change", "count": len(folder.get("files", [])), "files": folder.get("files", [])}
