  ...
  useEffect(() => {
    if (folder) {
      ...
      const coverPhoto = coverPhotoId
        ? `https://lh3.googleusercontent.com/d/${coverPhotoId}=w1000`
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 p-3 space-y-3 animate-pulse">
