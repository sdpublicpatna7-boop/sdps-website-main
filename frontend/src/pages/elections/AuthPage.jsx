...
import { getElectionSettings, getVoterDetails } from "../../lib/api_elections";
...
export default function AuthPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      reset();
      const data = await getVoterDetails(trimmed);
      if (data.has_voted) {
      ...
      setStudent(data);
      navigate("/elections/confirm");
    } catch (err) {
      toast.error(err?.message || "ID not found");
    } finally {
      setLoading(false);
    }
  };
